    ...
    struct example_deferred_render_singleton_t *instance)
{
    ...
    for (kan_memory_size_t index = 0u; index < POINT_LIGHTS_SHADOW_PASS_COUNT; ++index)
    {
...
static bool try_render_ground (struct deferred_render_state_t *state,
...
{
    KAN_UMI_VALUE_READ_OPTIONAL (ground_material_instance, kan_render_material_instance_loaded_t, name,
                                 &config->ground_material_instance_name)

    if (!ground_material_instance)
    {
        return false;
    }
    ...
    struct kan_float_matrix_4x4_t ground_transform_matrix = kan_transform_3_to_float_matrix_4x4 (&ground_transform);
    ...
    kan_render_pass_instance_attributes (pass_instance, expected_attribute_source->binding,
                                         sizeof (attribute_buffers) / sizeof (attribute_buffers[0u]), attribute_buffers,
                                         attribute_buffers_offsets);
...
static bool try_render_cubes (struct deferred_render_state_t *state,
                              struct example_deferred_render_singleton_t *singleton,
                              kan_render_pass_instance_t pass_instance,
                              kan_render_pipeline_parameter_set_t pass_parameter_set,
                              kan_interned_string_t pass_name,
                              const struct deferred_render_config_t *config)
{
    KAN_UMI_VALUE_READ_OPTIONAL (cube_material_instance, kan_render_material_instance_loaded_t, name,
                                 &config->cube_material_instance_name)

    if (!cube_material_instance)
    {
        return false;
    }
...
static inline bool try_render_opaque_objects (struct deferred_render_state_t *state,
                                              struct example_deferred_render_singleton_t *singleton,
                                              kan_render_pass_instance_t pass_instance,
                                              kan_render_pipeline_parameter_set_t pass_parameter_set,
                                              kan_interned_string_t pass_name,
                                              const struct deferred_render_config_t *config)
{
    return singleton->object_buffers_initialized &&
           try_render_ground (state, singleton, pass_instance, pass_parameter_set, pass_name, config) &&
           try_render_cubes (state, singleton, pass_instance, pass_parameter_set, pass_name, config);
}

static bool try_render_ambient_lighting (struct deferred_render_state_t *state,
                                         struct example_deferred_render_singleton_t *singleton,
                                         kan_render_pass_instance_t pass_instance,
...
static bool try_render_directional_lighting (struct deferred_render_state_t *state,
                                             struct example_deferred_render_singleton_t *singleton,
                                             kan_render_pass_instance_t pass_instance,
                                             struct deferred_render_scene_view_data_t *scene_view_data,
...
static bool try_render_point_lighting (struct deferred_render_state_t *state,
                                       struct example_deferred_render_singleton_t *singleton,
                                       kan_render_pass_instance_t pass_instance,
                                       struct deferred_render_scene_view_data_t *scene_view_data,
...
#define SHADOWLESS_POINT_LIGHTS_Y 31u
    ...
    for (kan_instance_size_t light_x = 0u; light_x < SHADOWLESS_POINT_LIGHTS_X; ++light_x)
    {
        for (kan_instance_size_t light_y = 0u; light_y < SHADOWLESS_POINT_LIGHTS_Y; ++light_y)
        {
            ...
            instanced_data_output->position_and_distance =
                kan_make_float_vector_4_t (light_min_x + light_step_x * (float) light_x + light_offset, 0.5f,
    ...
    kan_render_pass_instance_indices (pass_instance, singleton->full_screen_quad_index_buffer);
    kan_render_pass_instance_draw (pass_instance, 0u, singleton->full_screen_quad_index_count, 0u, 0u, total_lights);
    return true;
}
...
#define MAX_EXPECTED_SCENE_FRAME_BUFFER_REQUESTS 16u
    ...
    image_requests[DEFERRED_RENDER_SCENE_IMAGE_POSITION] = (struct kan_render_graph_resource_image_request_t) {
        .description =
            {
                .format = g_buffer_position_attachment->format,
    ...
    image_requests[DEFERRED_RENDER_SCENE_IMAGE_NORMAL_SPECULAR] = (struct kan_render_graph_resource_image_request_t) {
        .description =
            {
                .format = g_buffer_normal_shininess_attachment->format,
                .width = viewport_width,
                .height = viewport_height,
                .depth = 1u,
                .layers = 1u,
    ...
    image_requests[DEFERRED_RENDER_SCENE_IMAGE_ALBEDO] = (struct kan_render_graph_resource_image_request_t) {
        .description =
            {
                .format = g_buffer_diffuse_attachment->format,
                .width = viewport_width,
                .height = viewport_height,
                .depth = 1u,
                .layers = 1u,
    ...
    image_requests[DEFERRED_RENDER_SCENE_IMAGE_DEPTH] = (struct kan_render_graph_resource_image_request_t) {
        .description =
            {
                .format = g_buffer_depth_attachment->format,
                .width = viewport_width,
                .height = viewport_height,
                .depth = 1u,
                .layers = 1u,
    ...
    for (kan_memory_size_t index = 0u; index < SPLIT_SCREEN_VIEWS; ++index)
    {
        if (!KAN_HANDLE_IS_VALID (singleton->scene_view[index].g_buffer_parameter_set))
        {
            initialize_scene_view_buffer_if_needed (singleton, render_context, index,
                                                    g_buffer_scene_view_buffer_meta->main_size);

            struct kan_render_parameter_update_description_t bindings[] = {{
                .binding = g_buffer_scene_view_buffer_meta->binding,
                .buffer_binding =
                    {
                        .buffer = singleton->scene_view[index].view_parameters_buffer,
                        .offset = 0u,
                        .range = g_buffer_scene_view_buffer_meta->main_size,
                    },
            }};

            struct kan_render_pipeline_parameter_set_description_t description = {
                .layout = g_buffer_pass_variant->pass_parameter_set_layout,
                .stable_binding = true,
                .initial_bindings_count = sizeof (bindings) / sizeof (bindings[0u]),
                .initial_bindings = bindings,
    ...
    struct kan_rpl_meta_buffer_t *lighting_scene_view_buffer_meta =
        &((struct kan_rpl_meta_buffer_t *) lighting_pass_variant->pass_parameter_set_bindings.buffers.data)[0u];

    struct kan_rpl_meta_sampler_t *lighting_color_sampler_meta =
        &((struct kan_rpl_meta_sampler_t *) lighting_pass_variant->pass_parameter_set_bindings.samplers.data)[0u];

    struct kan_rpl_meta_sampler_t *lighting_depth_sampler_meta =
        &((struct kan_rpl_meta_sampler_t *) lighting_pass_variant->pass_parameter_set_bindings.samplers.data)[1u];

    struct kan_rpl_meta_image_t *lighting_world_position_image_meta =
        &((struct kan_rpl_meta_image_t *) lighting_pass_variant->pass_parameter_set_bindings.images.data)[0u];

    struct kan_rpl_meta_image_t *lighting_normal_shininess_image_meta =
        &((struct kan_rpl_meta_image_t *) lighting_pass_variant->pass_parameter_set_bindings.images.data)[1u];
    ...
    for (kan_memory_size_t index = 0u; index < SPLIT_SCREEN_VIEWS; ++index)
    {
        ...
        {
            initialize_scene_view_buffer_if_needed (singleton, render_context, index,
                                                    lighting_scene_view_buffer_meta->main_size);

            struct kan_render_parameter_update_description_t bindings[] = {
                {
                    .binding = lighting_scene_view_buffer_meta->binding,
                    .buffer_binding =
                        {
                            .buffer = singleton->scene_view[index].view_parameters_buffer,
                            .offset = 0u,
                            .range = lighting_scene_view_buffer_meta->main_size,
                        },
                },
                {
                    .binding = lighting_color_sampler_meta->binding,
                    .sampler_binding =
                        {
                            .sampler =
                                {
                ...
                {
                    .binding = lighting_depth_sampler_meta->binding,
                    .sampler_binding =
                        {
                            .sampler =
                                {
            ...
            struct kan_render_pipeline_parameter_set_description_t description = {
                .layout = lighting_pass_variant->pass_parameter_set_layout,
    ...
    image_requests[DEFERRED_RENDER_SHADOW_IMAGE_DEPTH] = (struct kan_render_graph_resource_image_request_t) {
        .description =
            {
                .format = shadow_depth_attachment->format,
    ...
    struct kan_render_graph_resource_request_t shadow_request = {
        .context = render_context->render_context,
    ...
    if (!(directional_light_shadow_response =
              kan_render_graph_resource_management_singleton_request (render_resource_management, &shadow_request)))
    {
        KAN_LOG (application_framework_example_deferred_render, KAN_LOG_ERROR,
    ...
    {
        shadow_request.frame_buffers_count = DEFERRED_RENDER_CUBE_SHADOW_FRAME_BUFFER_COUNT;
        image_requests[DEFERRED_RENDER_SHADOW_IMAGE_DEPTH] = (struct kan_render_graph_resource_image_request_t) {
            .description =
                {
                    .format = shadow_depth_attachment->format,
        ...
        for (kan_memory_size_t frame_buffer_index = 0u;
    ...
    struct kan_rpl_meta_buffer_t *shadow_pass_buffer_meta =
        &((struct kan_rpl_meta_buffer_t *) shadow_pass_variant->pass_parameter_set_bindings.buffers.data)[0u];
    initialize_shadow_pass_data_if_needed (render_context, &singleton->directional_light_shadow_pass,
                                           shadow_pass_variant, shadow_pass_buffer_meta);

    for (kan_memory_size_t light_index = 0u; light_index < POINT_LIGHTS_SHADOW_PASS_COUNT; ++light_index)
    {
    ...
    KAN_UMI_VALUE_READ_OPTIONAL (directional_material, kan_render_material_loaded_t, name,
    ...
    KAN_UMI_VALUE_READ_OPTIONAL (point_material, kan_render_material_loaded_t, name, &config->point_light_material_name)
    ...
    for (kan_memory_size_t index = 0u; index < SPLIT_SCREEN_VIEWS; ++index)
    {
        ...
        struct kan_render_parameter_update_description_t lighting_bindings[] = {
            {
                .binding = lighting_world_position_image_meta->binding,
                .image_binding =
                    {
            ...
            {
                .binding = lighting_normal_shininess_image_meta->binding,
                .image_binding =
                    {
            ...
            {
                .binding = lighting_diffuse_image_meta->binding,
                .image_binding =
                    {
        ...
        kan_render_pass_instance_t g_buffer_pass_instance = kan_render_pass_instantiate (
            g_buffer_pass->pass, scene_responses[index]->frame_buffers[DEFERRED_RENDER_SCENE_FRAME_BUFFER_G_BUFFER],
        ...
        KAN_UMI_VALUE_READ_OPTIONAL (ambient_material, kan_render_material_loaded_t, name,
                                     &config->ambient_light_material_name)

        everything_rendered &=
            ambient_material && try_render_ambient_lighting (state, singleton, lighting_pass_instance,
                                                             &singleton->scene_view[index], ambient_material);

        everything_rendered &= directional_material &&
                               try_render_directional_lighting (state, singleton, lighting_pass_instance,
                                                                &singleton->scene_view[index], directional_material);

        everything_rendered &=
            point_material && try_render_point_lighting (state, singleton, lighting_pass_instance,
                                                         &singleton->scene_view[index], point_material);
    ...
    for (kan_memory_size_t point_light_index = 0u; point_light_index < POINT_LIGHTS_WITH_SHADOWS; ++point_light_index)
    {
        ...
        for (kan_memory_size_t side = 0u; side < DEFERRED_RENDER_CUBE_SHADOW_FRAME_BUFFER_COUNT; ++side)
        {
            ...
            *pass_buffer_data = kan_float_matrix_4x4_multiply (&projection, &view);
            ...
            kan_render_pass_instance_t pass_instance = kan_render_pass_instantiate (
                shadow_pass->pass, light_response->frame_buffers[side], &viewport_bounds, &scissor, clear_values);
