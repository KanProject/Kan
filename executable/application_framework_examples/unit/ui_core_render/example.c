#include <application_framework_examples_ui_core_render_api.h>
...
KAN_LOG_DEFINE_CATEGORY (application_framework_example_ui_core_render);
KAN_USE_STATIC_INTERNED_IDS

#define KAN_UI_CORE_RENDER_MUTATOR_GROUP "ui_core_render"
KAN_UM_ADD_MUTATOR_TO_FOLLOWING_GROUP (ui_core_render_update)
KAN_UM_ADD_MUTATOR_TO_FOLLOWING_GROUP (ui_core_render_pre_shape)
KAN_UM_ADD_MUTATOR_TO_FOLLOWING_GROUP (ui_core_render_post_render)
APPLICATION_FRAMEWORK_EXAMPLES_UI_CORE_RENDER_API KAN_UM_MUTATOR_GROUP_META (ui_core_render,
                                                                             KAN_UI_CORE_RENDER_MUTATOR_GROUP);

#define TEST_WIDTH 1600u
#define TEST_HEIGHT 800u

struct ui_core_render_element_to_text_sync_t
{
    kan_ui_node_id_t node;
    kan_text_shaping_unit_id_t unit;
};

struct example_ui_core_render_singleton_t
{
...
APPLICATION_FRAMEWORK_EXAMPLES_UI_CORE_RENDER_API void example_ui_core_render_singleton_init (
    struct example_ui_core_render_singleton_t *instance)
{
...
struct ui_core_render_update_state_t
{
    KAN_UM_GENERATE_STATE_QUERIES (ui_core_render_update)
    KAN_UM_BIND_STATE (ui_core_render_update, state)
...
APPLICATION_FRAMEWORK_EXAMPLES_UI_CORE_RENDER_API KAN_UM_MUTATOR_DEPLOY (ui_core_render_update)
{
...
static void build_playground_ui (struct ui_core_render_update_state_t *state,
                                 struct example_ui_core_render_singleton_t *singleton)
{
    ...
    KAN_UML_SEQUENCE_DELETE (text_unit, kan_text_shaping_unit_t) { KAN_UM_ACCESS_DELETE (text_unit); }
    KAN_UML_SEQUENCE_DELETE (text_sync, ui_core_render_element_to_text_sync_t) { KAN_UM_ACCESS_DELETE (text_sync); }

    const kan_instance_size_t test_font_size = 24u;
#define ADD_TEXT_SHAPING_UNIT(NAME, ALIGNMENT, LITERAL)                                                                \
    ...
    {                                                                                                                  \
        ...
        NAME##_unit->request.font_size = test_font_size;                                                               \
    ...
    ADD_TEXT_SHAPING_UNIT (window_top_text, KAN_TEXT_SHAPING_ALIGNMENT_LEFT,
                           "Centered window with nine-slice buttons for tests.")
    ADD_TEXT_SHAPING_UNIT (window_remark_text, KAN_TEXT_SHAPING_ALIGNMENT_LEFT,
                           "Button colors are selected from color table by the way.")
    ADD_TEXT_SHAPING_UNIT (window_bottom_text, KAN_TEXT_SHAPING_ALIGNMENT_RIGHT,
                           "This text should always be at the bottom.")

    ADD_TEXT_SHAPING_UNIT (evil_color_text, KAN_TEXT_SHAPING_ALIGNMENT_CENTER, "Evil color!")
    ADD_TEXT_SHAPING_UNIT (good_color_text, KAN_TEXT_SHAPING_ALIGNMENT_CENTER, "Good color!")
    ADD_TEXT_SHAPING_UNIT (neutral_color_text, KAN_TEXT_SHAPING_ALIGNMENT_CENTER, "Neutral!")

    const kan_text_shaping_unit_id_t icons_test_text_id = kan_next_text_shaping_unit_id (text_shaping);
    KAN_UMO_INDEXED_INSERT (icons_test_text_unit, kan_text_shaping_unit_t)
    {
        icons_test_text_unit->id = icons_test_text_id;
        icons_test_text_unit->request.font_size = test_font_size;
    ...
    const kan_text_shaping_unit_id_t button_texts[] = {evil_color_text_id, good_color_text_id, neutral_color_text_id};
    ...
    KAN_UMO_INDEXED_INSERT (window_node, kan_ui_node_t)
    {
        ...
        KAN_UMO_INDEXED_INSERT (top_text_node, kan_ui_node_t)
        {
            ...
            top_text_node->element.height = KAN_UI_VALUE_PT ((float) test_font_size / 0.75f);
            ...
            KAN_UMO_INDEXED_INSERT (top_text_drawable, kan_ui_node_drawable_t)
            {
                ...
                top_text_drawable->main_draw_command.text.shaping_unit = window_top_text_id;
                top_text_drawable->main_draw_command.text.ui_mark = 0u;
                top_text_drawable->main_draw_command.text.animation_start_time_s = 0.0f;
            }

            KAN_UMO_INDEXED_INSERT (sync, ui_core_render_element_to_text_sync_t)
            {
                top_text_node->event_on_laid_out = true;
                sync->node = top_text_node->id;
                sync->unit = window_top_text_id;
            }
        }
        ...
        KAN_UMO_INDEXED_INSERT (remark_text_node, kan_ui_node_t)
        {
            ...
            remark_text_node->element.height = KAN_UI_VALUE_PT ((float) test_font_size / 0.75f);
            ...
            KAN_UMO_INDEXED_INSERT (remark_text_drawable, kan_ui_node_drawable_t)
            {
                ...
                remark_text_drawable->main_draw_command.text.shaping_unit = window_remark_text_id;
            ...
            KAN_UMO_INDEXED_INSERT (sync, ui_core_render_element_to_text_sync_t)
            {
                remark_text_node->event_on_laid_out = true;
                sync->node = remark_text_node->id;
                sync->unit = window_remark_text_id;
            }
        }
        ...
        KAN_UMO_INDEXED_INSERT (icons_test_text_node, kan_ui_node_t)
        {
            ...
            icons_test_text_node->element.height = KAN_UI_VALUE_PT ((float) test_font_size / 0.75f);
            ...
            KAN_UMO_INDEXED_INSERT (icons_test_text_drawable, kan_ui_node_drawable_t)
            {
                ...
                icons_test_text_drawable->main_draw_command.text.shaping_unit = icons_test_text_id;
            ...
            KAN_UMO_INDEXED_INSERT (sync, ui_core_render_element_to_text_sync_t)
            {
                icons_test_text_node->event_on_laid_out = true;
                sync->node = icons_test_text_node->id;
                sync->unit = icons_test_text_id;
            }
        }
        ...
        KAN_UMO_INDEXED_INSERT (bottom_text_node, kan_ui_node_t)
        {
            ...
            bottom_text_node->element.height = KAN_UI_VALUE_PT ((float) test_font_size / 0.75f);
            ...
            KAN_UMO_INDEXED_INSERT (bottom_text_drawable, kan_ui_node_drawable_t)
            {
                ...
                bottom_text_drawable->main_draw_command.text.shaping_unit = window_bottom_text_id;
            ...
            KAN_UMO_INDEXED_INSERT (sync, ui_core_render_element_to_text_sync_t)
            {
                bottom_text_node->event_on_laid_out = true;
                sync->node = bottom_text_node->id;
                sync->unit = window_bottom_text_id;
            }
        }
                    ...
                    for (kan_loop_size_t button = 0u; button < 6u; ++button)
                    {
                        KAN_UMO_INDEXED_INSERT (button_node, kan_ui_node_t)
                        {
                            ...
                            KAN_UMO_INDEXED_INSERT (button_text_node, kan_ui_node_t)
                            {
                                ...
                                button_text_node->element.height = KAN_UI_VALUE_PT ((float) test_font_size / 0.75f);
                                ...
                                KAN_UMO_INDEXED_INSERT (button_text_drawable, kan_ui_node_drawable_t)
                                {
                                    ...
                                    button_text_drawable->main_draw_command.text.shaping_unit =
                                ...
                                if (column == 0u && button < button_type_count)
                                {
                                    KAN_UMO_INDEXED_INSERT (sync, ui_core_render_element_to_text_sync_t)
                                    {
                                        button_text_node->event_on_laid_out = true;
                                        sync->node = button_text_node->id;
                                        sync->unit = button_texts[button & button_type_count];
                                    }
                                }
                            }
                        }
...
APPLICATION_FRAMEWORK_EXAMPLES_UI_CORE_RENDER_API KAN_UM_MUTATOR_EXECUTE (ui_core_render_update)
{
    ...
    KAN_UMI_SINGLETON_WRITE (singleton, example_ui_core_render_singleton_t)
    ...
    if (!KAN_HANDLE_IS_VALID (singleton->window_handle))
    {
        ...
        singleton->window_handle = kan_application_system_window_create (state->application_system_handle,
                                                                         "application_framework_example_ui_core_render",
        ...
        {
            KAN_LOG (application_framework_example_ui_core_render, KAN_LOG_ERROR, "Failed to create surface.")
...
struct ui_core_render_pre_shape_state_t
{
    KAN_UM_GENERATE_STATE_QUERIES (ui_core_render_pre_shape)
    KAN_UM_BIND_STATE (ui_core_render_pre_shape, state)
    kan_context_system_t application_system_handle;
};

APPLICATION_FRAMEWORK_EXAMPLES_UI_CORE_RENDER_API KAN_UM_MUTATOR_DEPLOY (ui_core_render_pre_shape)
{
    kan_static_interned_ids_ensure_initialized ();
    kan_context_t context = kan_universe_get_context (universe);
    state->application_system_handle = kan_context_query (context, KAN_CONTEXT_APPLICATION_SYSTEM_NAME);
...
APPLICATION_FRAMEWORK_EXAMPLES_UI_CORE_RENDER_API KAN_UM_MUTATOR_EXECUTE (ui_core_render_pre_shape)
{
    KAN_UML_EVENT_FETCH (event, kan_ui_node_laid_out_t)
    {
        KAN_UMI_VALUE_READ_OPTIONAL (sync, ui_core_render_element_to_text_sync_t, node, &event->node_id)
        if (sync)
        {
            KAN_UMI_VALUE_READ_OPTIONAL (drawable, kan_ui_node_drawable_t, id, &sync->node)
            KAN_UMI_VALUE_UPDATE_OPTIONAL (unit, kan_text_shaping_unit_t, id, &sync->unit)

            if (drawable && unit)
            {
                if (unit->request.primary_axis_limit != (kan_instance_size_t) drawable->width)
                {
                    unit->request.primary_axis_limit = (kan_instance_size_t) drawable->width;
                    unit->dirty = true;
                }
            }
        }
...
struct ui_core_render_post_render_state_t
{
    KAN_UM_GENERATE_STATE_QUERIES (ui_core_render_post_render)
    KAN_UM_BIND_STATE (ui_core_render_post_render, state)
    kan_context_system_t application_system_handle;
};

APPLICATION_FRAMEWORK_EXAMPLES_UI_CORE_RENDER_API KAN_UM_MUTATOR_DEPLOY (ui_core_render_post_render)
{
    kan_static_interned_ids_ensure_initialized ();
    kan_context_t context = kan_universe_get_context (universe);
    state->application_system_handle = kan_context_query (context, KAN_CONTEXT_APPLICATION_SYSTEM_NAME);
...
APPLICATION_FRAMEWORK_EXAMPLES_UI_CORE_RENDER_API KAN_UM_MUTATOR_EXECUTE (ui_core_render_post_render)
{
    ...
    if (KAN_HANDLE_IS_VALID (ui_render_graph->final_pass_instance) && KAN_HANDLE_IS_VALID (singleton->window_surface))
    {
        ...
        if (test->test_mode_enabled && test->expectation_read_back_statuses.size == 0u)
        {
