...
static inline void finish_node (kan_memory_size_t user_data)
{
    int index = kan_atomic_int_add (&finish_index, 1u);
    finish_nodes[index] = (const char *) user_data;
}

static inline kan_memory_size_t find_finished_node_index (const char *node_name)
{
    kan_memory_size_t node_finish_index = 0u;
    while (node_finish_index < (kan_memory_size_t) kan_atomic_int_get (&finish_index) &&
...
static void verify_checkpoint_dependency_up (kan_instance_size_t nodes_count,
                                             ...
                                             kan_memory_size_t root_node_finish_index,
                                             const char *checkpoint_name)
{
    for (kan_memory_size_t node_index = 0u; node_index < nodes_count; ++node_index)
    {
        struct node_t *node = &nodes[node_index];
        for (kan_memory_size_t depends_on_index = 0u; depends_on_index < node->depends_on_count; ++depends_on_index)
        {
            if (strcmp (node->depends_on[depends_on_index], checkpoint_name) == 0)
            {
                const kan_memory_size_t found_index = find_finished_node_index (node->name);
    ...
    for (kan_memory_size_t index = 0u; index < checkpoint_dependencies_count; ++index)
    {
                                               ...
                                               const char *checkpoint_name)
{
    ...
    {
        struct node_t *node = &nodes[node_index];
        for (kan_memory_size_t dependency_of_index = 0u; dependency_of_index < node->dependency_of_count;
             ++dependency_of_index)
        {
                               ...
                               struct checkpoint_dependency_t *checkpoint_dependencies)
{
    ...
    for (kan_memory_size_t index = 0u; index < nodes_count; ++index)
    {
        struct node_t *node = &nodes[index];
        kan_workflow_graph_node_t graph_node = kan_workflow_graph_node_create (builder, node->name);
        kan_workflow_graph_node_set_function (graph_node, node->function, (kan_memory_size_t) node->name);
    ...
    for (kan_memory_size_t execution_index = 0u; execution_index < 5u; ++execution_index)
    {
        ...
        {
            struct node_t *node = &nodes[node_index];
            const kan_memory_size_t node_finish_index = find_finished_node_index (node->name);

            if (node_finish_index == (kan_memory_size_t) kan_atomic_int_get (&finish_index))
            {
            ...
            for (kan_memory_size_t depends_on_index = 0u; depends_on_index < node->depends_on_count; ++depends_on_index)
            {
                const kan_memory_size_t found_index = find_finished_node_index (node->depends_on[depends_on_index]);
                if (found_index != (kan_memory_size_t) kan_atomic_int_get (&finish_index))
                {
            ...
            for (kan_memory_size_t dependency_of_index = 0u; dependency_of_index < node->dependency_of_count;
                 ++dependency_of_index)
            {
                const kan_memory_size_t found_index =
                    find_finished_node_index (node->dependency_of[dependency_of_index]);
                if (found_index != (kan_memory_size_t) kan_atomic_int_get (&finish_index))
                {
                    KAN_TEST_CHECK (node_finish_index < found_index)
                }
...
static void single_threaded_node_function (kan_cpu_job_t job, kan_memory_size_t user_data)
{
...
static void multi_threaded_sub_task (kan_memory_size_t user_data)
{
    ...
    {
        const kan_memory_size_t node_user_data = state->user_data;
        kan_free_batched (KAN_ALLOCATION_GROUP_IGNORE, state);
        finish_node (node_user_data);
    }
}

static void multi_threaded_node_function (kan_cpu_job_t job, kan_memory_size_t user_data)
{
    ...
    for (kan_memory_size_t index = 0u; index < MULTI_THREAD_NODE_SUB_TASKS; ++index)
    {
        kan_cpu_job_dispatch_task (job, (struct kan_cpu_task_t) {
                                            .function = multi_threaded_sub_task,
                                            .user_data = (kan_memory_size_t) state,
