...
///         const kan_floating_t node_max[] = {/* Fill max coordinates. */};
///
...
struct kan_space_tree_ray_iterator_t
{
    ...
    kan_floating_t position[KAN_CONTAINER_SPACE_TREE_MAX_DIMENSIONS];
    kan_floating_t direction[KAN_CONTAINER_SPACE_TREE_MAX_DIMENSIONS];
    kan_floating_t travelled_time;
...
CONTAINER_API struct kan_space_tree_insertion_iterator_t kan_space_tree_insertion_start (
    struct kan_space_tree_t *tree, const kan_floating_t *min_sequence, const kan_floating_t *max_sequence);
...
static inline bool kan_check_if_bounds_intersect (kan_instance_size_t dimension_count,
                                                  const kan_floating_t *first_min,
                                                  const kan_floating_t *first_max,
                                                  const kan_floating_t *second_min,
                                                  const kan_floating_t *second_max)
{
...
struct kan_ray_intersection_output_t
{
    bool hit;
    kan_floating_t time;
    kan_floating_t coordinates[KAN_CONTAINER_SPACE_TREE_MAX_DIMENSIONS];
};
...
static inline struct kan_ray_intersection_output_t kan_check_if_ray_and_bounds_intersect (
    kan_instance_size_t dimension_count,
    const kan_floating_t *bounds_min,
    const kan_floating_t *bounds_max,
    const kan_floating_t *ray_origin,
    const kan_floating_t *ray_direction)
{
    ...
    kan_floating_t candidate_plane[KAN_CONTAINER_SPACE_TREE_MAX_DIMENSIONS];
...
#define CASE(DIMENSION)                                                                                                \
    case (DIMENSION + 1u):                                                                                             \
        ...
        {                                                                                                              \
            kan_floating_t time = (candidate_plane[DIMENSION] - ray_origin[DIMENSION]) / ray_direction[DIMENSION];     \
            if (time > max_time)                                                                                       \
            {                                                                                                          \
                max_time = time;                                                                                       \
