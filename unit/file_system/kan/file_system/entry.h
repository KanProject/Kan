...
/// \brief Queries status of entry at given path. Returns true on success. Returns false if file does not exist.
FILE_SYSTEM_API bool kan_file_system_query_entry (const char *path, struct kan_file_system_entry_status_t *status);

/// \brief Queries whether entry at given path exists.
FILE_SYSTEM_API bool kan_file_system_check_existence (const char *path);

/// \brief Attempts to move file entry from one path to another. Returns true on success.
FILE_SYSTEM_API bool kan_file_system_move_file (const char *from, const char *to);

/// \brief Attempts to remove file entry at given path. Returns true on success.
FILE_SYSTEM_API bool kan_file_system_remove_file (const char *path);

/// \brief Attempts to create directory entry at given path. Returns true on success.
/// \warning We treat failure due to the existence of directory as success, as it is much more convenient for tools.
FILE_SYSTEM_API bool kan_file_system_make_directory (const char *path);

/// \brief Attempts to remove directory entry and all its children at given path. Returns true on success.
FILE_SYSTEM_API bool kan_file_system_remove_directory_with_content (const char *path);
...
/// \details
FILE_SYSTEM_API bool kan_file_system_lock_file_create (const char *path, enum kan_file_system_lock_file_flags_t flags);
