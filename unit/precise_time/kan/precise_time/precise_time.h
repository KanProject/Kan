...
PRECISE_TIME_API kan_stable_size_t kan_precise_time_get_elapsed_nanoseconds (void);

/// \brief Returns count of nanoseconds since Unix epoch in nanoseconds in UTC zone.
PRECISE_TIME_API kan_stable_size_t kan_precise_time_get_epoch_nanoseconds_utc (void);

/// \brief Transfers current thread to sleeping state for given amount of nanoseconds.
PRECISE_TIME_API void kan_precise_time_sleep (kan_stable_size_t nanoseconds);
