...
struct random_access_stream_buffer_t
{
    ...
    kan_stable_size_t stream_position;
    kan_stable_size_t stream_size;
    kan_stable_size_t buffer_position;
    kan_stable_size_t buffer_size;
    uint8_t buffer[];
};
...
static inline bool fill_buffer (struct random_access_stream_buffer_t *data)
{
    ...
    const kan_stable_size_t to_read_into_buffer =
        KAN_MIN (data->stream_size - data->stream_position, data->buffer_size);
    return data->source_stream->operations->read (data->source_stream, to_read_into_buffer, data->buffer) ==
           to_read_into_buffer;
}

static kan_stable_size_t buffered_read (struct kan_stream_t *stream, kan_stable_size_t amount, void *output_buffer)
{
    struct random_access_stream_buffer_t *data = (struct random_access_stream_buffer_t *) stream;
    const kan_stable_size_t buffer_begin = data->buffer_position;
    const kan_stable_size_t buffer_end = buffer_begin + data->buffer_size;

    kan_stable_size_t request_begin = data->stream_position;
    const kan_stable_size_t request_end = request_begin + amount;
    uint8_t *output = (uint8_t *) output_buffer;
    kan_stable_size_t total_read = 0u;

    if (request_begin >= (kan_stable_size_t) buffer_begin && request_begin < (kan_stable_size_t) buffer_end)
    {
        const kan_stable_size_t begin_buffer_offset = request_begin - buffer_begin;
        if (request_end < (kan_stable_size_t) buffer_end)
        {
            const kan_stable_size_t buffered_amount = KAN_MIN (data->stream_size, request_end) - request_begin;
            ...
            return (kan_stable_size_t) buffered_amount;
        }
        else
        {
            const kan_stable_size_t buffered_amount = KAN_MIN (data->stream_size, buffer_end) - request_begin;
            memcpy (output, data->buffer + begin_buffer_offset, buffered_amount);
            output += buffered_amount;
            request_begin += buffered_amount;
            total_read += (kan_stable_size_t) buffered_amount;
            data->stream_position += buffered_amount;
        }
    }
    else if (request_begin < buffer_begin && request_end >= buffer_begin)
    {
        const kan_stable_size_t to_read_unbuffered = buffer_begin - request_begin;
        data->source_stream->operations->seek (data->source_stream, KAN_STREAM_SEEK_START, data->stream_position);

        const kan_stable_size_t read =
            (kan_stable_size_t) data->source_stream->operations->read (data->source_stream, to_read_unbuffered, output);
        total_read += read;
        output += read;
        ...
        {
            const kan_stable_size_t buffered_amount = KAN_MIN (data->stream_size, request_end) - buffer_begin;
            ...
            return (kan_stable_size_t) total_read;
        }
        else
        {
            const kan_stable_size_t buffered_amount =
    ...
    if (request_end - request_begin > data->buffer_size)
    {
        ...
        data->source_stream->operations->seek (data->source_stream, KAN_STREAM_SEEK_START, data->stream_position);
        const kan_stable_size_t read = (kan_stable_size_t) data->source_stream->operations->read (
...
static inline kan_stable_size_t write_to_buffer (struct random_access_stream_buffer_t *data,
...
static kan_stable_size_t buffered_write (struct kan_stream_t *stream,
                                         kan_stable_size_t amount,
                                         const void *input_buffer)
{
    struct random_access_stream_buffer_t *data = (struct random_access_stream_buffer_t *) stream;
    ...
    kan_stable_size_t buffer_end = data->buffer_position + data->buffer_size;
    kan_stable_size_t available = buffer_end - data->stream_position;
    kan_stable_size_t written;
...
static bool buffered_flush (struct kan_stream_t *stream)
{
    ...
    if (to_write > 0u)
    {
        ...
        const kan_stable_size_t written =
...
static kan_stable_size_t buffered_tell (struct kan_stream_t *stream)
{
    struct random_access_stream_buffer_t *data = (struct random_access_stream_buffer_t *) stream;
    return data->stream_position;
}

static bool buffered_seek (struct kan_stream_t *stream, enum kan_stream_seek_pivot pivot, kan_stable_offset_t offset)
{
    struct random_access_stream_buffer_t *data = (struct random_access_stream_buffer_t *) stream;
    ...
    case KAN_STREAM_SEEK_START:
        KAN_ASSERT (offset >= 0)
        data->stream_position = (kan_stable_size_t) offset;
    ...
    {
        kan_stable_offset_t position_signed = (kan_stable_offset_t) data->stream_position;
        ...
        else if (position_signed > (kan_stable_offset_t) data->stream_size)
        {
            position_signed = (kan_stable_offset_t) data->stream_size;
        }

        data->stream_position = (kan_stable_size_t) position_signed;
        break;
    }

    case KAN_STREAM_SEEK_END:
    {
        kan_stable_offset_t position_signed = (kan_stable_offset_t) data->stream_size;
        position_signed += offset;
